...
def sql_placeholders(sql):
    """
    The original OrderFlow code used SQLite-style ? placeholders.
    PostgreSQL/psycopg uses %s.

    This lets the rest of the application keep the same query style.
    """
...
# ============================================================
# CURRENT USER / ORGANISATION / SITE
# ============================================================
...
def finance_summary(month=None):
    ...
    revenue_row = q(
        """
        SELECT COALESCE(SUM(gross),0) AS x
        FROM sales
        WHERE organisation_id=?
          AND site_id=?
          AND sale_date>=?
    ...
    cogs_row = q(
        """
        SELECT COALESCE(
            SUM(ABS(sm.quantity) * si.unit_cost),
            0
        ) AS x
        FROM stock_movements sm
        JOIN stock_items si
          ON si.id=sm.stock_item_id
        WHERE sm.organisation_id=?
          AND sm.site_id=?
          AND sm.created_at>=?
          AND sm.created_at<?
          AND LOWER(sm.movement_type)
              IN ('usage','waste','sale','cogs')
        """,
        (u["organisation_id"], s["id"], start, end),
        True,
    )
    ...
    labour_row = q(
        ...
        SELECT COALESCE(
            SUM(
                CASE
                    WHEN sh.status!='Cancelled'
                    THEN GREATEST(
                        0,
                        (
                            (
                                (
                                    CAST(substr(sh.end_time,1,2) AS INTEGER) * 60
                                    + CAST(substr(sh.end_time,4,2) AS INTEGER)
                                )
                                -
                                (
                                    CAST(substr(sh.start_time,1,2) AS INTEGER) * 60
                                    + CAST(substr(sh.start_time,4,2) AS INTEGER)
                                )
                            ) / 60.0
                        )
                        -
                        sh.break_minutes / 60.0
                    ) * e.pay_rate
                    ELSE 0
                END
            ),
        ...
        try:
            with conn() as c:
                with c.cursor() as cur:
                    cur.execute(
                        """
                        INSERT INTO organisations(name,business_type,created_at)
                        VALUES(%s,%s,%s)
                        RETURNING id
                        """,
                    ...
                    cur.execute(
                        """
                        INSERT INTO sites(organisation_id,name,address,created_at)
                        VALUES(%s,%s,%s,%s)
                        RETURNING id
                        """,
                    ...
                    cur.execute(
                        """
                        INSERT INTO users(
                            organisation_id,name,email,password_hash,role,created_at
                        )
                        VALUES(%s,%s,%s,%s,%s,%s)
                        RETURNING id
                        """,
...
def dashboard():
    ...
    payroll = q(
        """
        SELECT COALESCE(SUM(net_pay),0) AS x
        FROM payroll_runs
        WHERE organisation_id=?
          AND site_id=?
...
def add_sale():
    ...
    sale_id = execute(
        """
        INSERT INTO sales(
            organisation_id,site_id,sale_date,category,net,vat,gross,source
        )
        VALUES(?,?,?,?,?,?,?,?)
        """,
        (
...
def add_expense():
    ...
    expense_id = execute(
        ...
        INSERT INTO expenses(
            organisation_id,site_id,expense_date,category,description,supplier,net,vat,gross
        )
        VALUES(?,?,?,?,?,?,?,?,?)
        """,
        (
...
def add_invoice():
    ...
    invoice_id = execute(
        ...
        INSERT INTO invoices(
            organisation_id,site_id,supplier,invoice_number,invoice_date,due_date,
            category,net,vat,gross,notes,created_at
        )
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?)
        """,
        (
...
def approve_invoice(iid):
    ...
    payment_id = execute(
        ...
        INSERT INTO payments(
            organisation_id,site_id,payment_type,payee,reference,amount,payment_date,
            status,method,source_id,approved_by,approved_at
        )
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?)
        """,
        (
...
def add_employee():
    ...
    employee_id = execute(
        ...
        INSERT INTO employees(
            organisation_id,site_id,name,email,department,job_title,pay_type,
            pay_rate,ni_rate,pension_rate,holiday_allowance
        )
        VALUES(?,?,?,?,?,?,?,?,?,?,?)
        """,
        (
...
def get_shifts():
    ...
    result = q(
        """
        SELECT sh.*,e.name AS employee_name,e.pay_rate
        FROM shifts sh
...
def add_shift():
    ...
    shift_id = execute(
        """
        INSERT INTO shifts(
            organisation_id,site_id,employee_id,shift_date,start_time,end_time,break_minutes
        )
        VALUES(?,?,?,?,?,?,?)
        """,
        (
...
def add_payroll():
    ...
    run_id = execute(
        ...
        INSERT INTO payroll_runs(
            organisation_id,site_id,period_start,period_end,gross_pay,
            employer_costs,deductions,net_pay,created_at
        )
        VALUES(?,?,?,?,?,?,?,?,?)
        """,
        (
...
def add_stock():
    ...
    item_id = execute(
        ...
        INSERT INTO stock_items(
            organisation_id,site_id,name,category,unit,on_hand,par_level,unit_cost,supplier
        )
        VALUES(?,?,?,?,?,?,?,?,?)
        """,
        (
...
def stock_move(iid):
    ...
    execute(
        ...
        INSERT INTO stock_movements(
            organisation_id,site_id,stock_item_id,quantity,movement_type,note,created_at
        )
        VALUES(?,?,?,?,?,?,?)
        """,
        (
...
def add_supplier():
    ...
    supplier_id = execute(
        ...
        INSERT INTO suppliers(
            organisation_id,name,contact,email,phone,payment_terms
        )
        VALUES(?,?,?,?,?,?)
        """,
        (
...
def add_menu():
    ...
    menu_id = execute(
        ...
        INSERT INTO menu_items(
            organisation_id,site_id,name,category,selling_price,recipe_cost
        )
        VALUES(?,?,?,?,?,?)
        """,
        (
    ...
    with conn() as c:
        with c.cursor() as cur:
            cur.execute(
                """
                INSERT INTO budgets(
                    organisation_id,site_id,month,revenue,food_cost,
                    drink_cost,labour,overheads
                )
                ...
                DO UPDATE SET
                    ...
                    drink_cost=EXCLUDED.drink_cost,
...
def weather():
    ...
    if lat is None or lon is None:
        ...
        if not address:
            return jsonify(
                error="Location unavailable. Allow location access or add a site address."
            ), 400
        ...
        try:
            ...
            if not results:
                return jsonify(
                    error=(
                        "Could not find the site location. Allow browser location "
                        "access or update the site address."
                    )
                ), 400
...
def add_event():
    ...
    event_id = execute(
        """
        INSERT INTO events(
            organisation_id,site_id,title,event_date,start_time,end_time,
            event_type,notes,all_day,created_by,created_at
        )
        VALUES(?,?,?,?,?,?,?,?,?,?,?)
        """,
        (
